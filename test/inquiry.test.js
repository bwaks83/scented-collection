...
test('Only selected product and relevant customization answers survive',()=>{
 const a=cleanAnswers({product:'Room Sprays',spray_size:'100ml (3.38 oz)',candle_size:'500g',logo:'No',logo_color:'red',logo_background:'Yes',background_color:'blue',packaging:'No',packaging_colors:'green'});
...
test('Invalid input and soap quantities rejected',()=>{
 for(const data of [null,{product:'x'},{product:'Candles',comments:'a'.repeat(3001)},{product:'Liquid Soap',soap_quantity:'Other',soap_other:'499'}])assert.throws(()=>cleanAnswers(data));
 assert.equal(cleanAnswers({product:'Liquid Soap',soap_quantity:'Other',soap_other:'500'}).soap_other,'500');
});
test('API returns success only after upstream confirms storage',async()=>{
 ...
 const req={method:'POST',headers:{host:'scented-collection.vercel.app',origin:'https://scented-collection.vercel.app','content-type':'application/json'},body:{submissionId:'12345678-1234-1234-1234-123456789abc',answers:{product:'Candles'}}};
 try{
